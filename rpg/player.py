...
class Player(BaseSprite):
    ...
    def __init__(self, pos: Tuple[float, float], groups: List[pygame.sprite.Group], sound_manager: Any, particles: Any) -> None:
        ...
        self.out_of_combat_timer: float = 5.0  # seconds since last combat action
        self.hp_regen_accumulator: float = 0.0
    ...
    def level_up(self) -> None:
        ...
        if hasattr(self, "game") and hasattr(self.game, "event_bus"):
            self.game.event_bus.emit("level_up", level=self.level, player=self, game=self.game)
            self.game.event_bus.emit("first_levelup")
    ...
    def perform_attack(self) -> None:
        ...
        if hasattr(self, "game") and hasattr(self.game, "event_bus"):
            self.game.event_bus.emit("first_combat")
    ...
    def update(self, dt: float) -> None:
        ...
        if hasattr(self, "game") and self.game and hasattr(self.game, "enemies"):
            for e in self.game.enemies:
                if e.hp > 0 and getattr(e, "state", "") in ["chase", "attack"]:
                    dist_sq = (e.pos - self.pos).length_squared()
                    if dist_sq < 280 * 280:  # Within active combat radius
        ...
        if self.out_of_combat_timer >= 4.0 and self.hp < self.max_hp and self.state != "dead":
            ...
            regen_mult = getattr(diff_prof, "hp_regen_mult", 1.0)
            ...
            if self.regen_sparkle_timer >= 0.4:
                self.regen_sparkle_timer = 0.0
                if hasattr(self, "particles") and self.particles:
                    from rpg.particles import Particle
                    for _ in range(3):
                        self.particles.add_particle(Particle(
                            pos=pygame.math.Vector2(
                                self.hitbox.centerx + random.uniform(-14, 14),
                                self.hitbox.centery + random.uniform(-4, 12)
                            ),
                            velocity=pygame.math.Vector2(random.uniform(-10, 10), random.uniform(-45, -20)),
                            color=random.choice([(60, 235, 120), (100, 245, 160), (40, 210, 230)]),
