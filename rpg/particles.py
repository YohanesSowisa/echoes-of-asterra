...
class ParticleManager:
    ...
    def create_sparkle(self, pos: Tuple[float, float], color: Tuple[int, int, int] = (255, 215, 0), count: int = 8) -> None:

        """Spawns radial magical sparkles around position."""
        pos_vec = pygame.math.Vector2(pos)
        for _ in range(count):
            angle = random.uniform(0, 6.28)
            speed = random.uniform(40, 140)
            self.add_particle(Particle(
                pos=pos_vec + pygame.math.Vector2(random.uniform(-4, 4), random.uniform(-4, 4)),
                velocity=pygame.math.Vector2(math.cos(angle) * speed, math.sin(angle) * speed),
                color=color,
                size=random.uniform(2, 4),
                lifetime=random.uniform(0.3, 0.6),
